...
def _substitute_term(term: TermName, mapping: dict[str, TermName]) -> TermName:
...
def standardize_variables(formula: Formula, counter: list[int] | None = None) -> Formula:
    ...
    return _standardize(formula, {}, counter)
...
def _standardize(
    ...
    if isinstance(formula, PredicateExpr):
        return PredicateExpr(
            formula.name,
            tuple(_substitute_term(arg, mapping) for arg in formula.args),
        )
    if isinstance(formula, ForAll):
        local_mapping = dict(mapping)
        fresh_variables: list[str] = []
        for variable in formula.variables:
            fresh = f"{variable}_{counter[0]}"
            counter[0] += 1
            local_mapping[variable] = fresh
            fresh_variables.append(fresh)
        return ForAll(tuple(fresh_variables), _standardize(formula.body, local_mapping, counter))
    if isinstance(formula, Exists):
        ...
        for variable in formula.variables:
            ...
            local_mapping[variable] = fresh
            fresh_variables.append(fresh)
        return Exists(tuple(fresh_variables), _standardize(formula.body, local_mapping, counter))
    if isinstance(formula, And):
        return And(tuple(_standardize(item, mapping, counter) for item in formula.operands))
    if isinstance(formula, Or):
        return Or(tuple(_standardize(item, mapping, counter) for item in formula.operands))
    if isinstance(formula, Not):
        return Not(_standardize(formula.operand, mapping, counter))
    if isinstance(formula, Implies):
        return Implies(
            _standardize(formula.left, mapping, counter),
            _standardize(formula.right, mapping, counter),
        )
    return formula
...
def _rename_formula(formula: Formula, mapping: dict[str, str]) -> Formula:
    if isinstance(formula, PredicateExpr):
        return PredicateExpr(formula.name, tuple(_substitute_term(arg, mapping) for arg in formula.args))
    if isinstance(formula, Not):
...
def skolemize(formula: Formula, skolem_counter: list[int] | None = None) -> Formula:
    ...
    return _skolemize(formula, [], skolem_counter)


def _skolemize(
    formula: Formula,
    universal_scope: list[str],
    skolem_counter: list[int],
) -> Formula:
    if isinstance(formula, Exists):
        mapping: dict[str, str] = {}
        for variable in formula.variables:
            if universal_scope:
                args = ", ".join(universal_scope)
                mapping[variable] = f"Sk{skolem_counter[0]}({args})"
            else:
                mapping[variable] = f"Sk{skolem_counter[0]}"
            skolem_counter[0] += 1
        return _skolemize(_rename_formula(formula.body, mapping), universal_scope, skolem_counter)
    if isinstance(formula, ForAll):
        next_scope = universal_scope + list(formula.variables)
        return ForAll(formula.variables, _skolemize(formula.body, next_scope, skolem_counter))
    if isinstance(formula, And):
        return And(tuple(_skolemize(item, universal_scope, skolem_counter) for item in formula.operands))
    if isinstance(formula, Or):
        return Or(tuple(_skolemize(item, universal_scope, skolem_counter) for item in formula.operands))
    if isinstance(formula, Not):
        return Not(_skolemize(formula.operand, universal_scope, skolem_counter))
    return formula
